...
async function runBasicTest() {
	...
	log("✅ 登录确认成功", "green");
	log(
		`用户信息: ${confirmResult.data.userInfo.name || confirmResult.data.userInfo.username}`,
		"cyan",
	);
...
async function runErrorTest() {
	...
	const invalidResult = await testAPI("/api/login/status", {
	...
	const notExistResult = await testAPI("/api/login/status", {
		sceneId: generateTestSceneId(),
	});
...
async function runAllTests() {
	...
	log("📊 测试结果总结:", "blue");
	log(
		`基础功能测试: ${results.basic ? "✅ 通过" : "❌ 失败"}`,
		results.basic ? "green" : "red",
	);
	log(
		`错误处理测试: ${results.error ? "✅ 通过" : "❌ 失败"}`,
		results.error ? "green" : "red",
	);
	log(
		`性能测试: ${results.performance ? "✅ 通过" : "❌ 失败"}`,
