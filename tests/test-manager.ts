...
class TestManager {
	...
	listTests() {
		...
		const categories = Array.from(
			new Set(this.testSuites.map((t) => t.category)),
		);
	...
	async runTest(testName: string) {
		const suite = this.testSuites.find(
			(t) => t.name.includes(testName) || t.name === testName,
		);
	...
	async runTestsByCategory(category: string) {
		...
		const categorySuites = this.testSuites.filter(
	...
	showHelp() {
		...
		this.log(
			"  npm run test:manager <category>   - 运行指定类别测试",
			"yellow",
		);
		this.log("");
		this.log("示例:", "green");
	...
	async runTestsByPattern(pattern: string) {
		const matchingSuites = this.testSuites.filter(
			(suite) =>
...
if (require.main === module) {
	...
	switch (command) {
		...
		default: {
			// 检查是否是测试类别
			const categories = Array.from(
				new Set(manager.testSuites.map((t) => t.category)),
			);
