			...
			<div className="space-y-6">
				...
				<div className="rounded-lg border bg-white p-6">
					<div className="mb-4 flex items-center gap-4">
						...
						<h1 className="font-bold text-2xl text-gray-900">⚡ 性能监控</h1>
					</div>
					<p className="text-gray-600">
						性能监控帮助开发者了解应用的性能表现，识别性能瓶颈并优化用户体验。 Sentry 提供了完整的性能监控解决方案，包括
						Web Vitals、数据库查询、API 响应时间等。
					</p>
				</div>

				{/* Web Vitals 指标 */}
				<div className="rounded-lg border bg-white p-6">
					<h2 className="mb-4 font-semibold text-xl">📊 Web Vitals 指标</h2>
								...
								<div className="flex items-start justify-between">
									<div>
										...
										<p className="mt-1 font-bold text-2xl">
											{metric.value}
											<span className="ml-1 font-normal text-gray-600 text-sm">{metric.unit}</span>
										</p>
									</div>
									<div
									...
									>
										{metric.status === "good" ? "良好" : metric.status === "warning" ? "警告" : "差"}
									</div>
								</div>
					...
					<div className="mt-4 flex gap-2">
						<button onClick={getWebVitals} className="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600">
							刷新指标
						</button>
						<button
							onClick={() => {
							...
							className={`rounded px-4 py-2 text-white ${isMonitoring ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"}`}
						>
				...
				{/* 性能测试控制台 */}
				<div className="rounded-lg border bg-white p-6">
					<h2 className="mb-4 font-semibold text-xl">🧪 性能测试控制台</h2>
				...
				{/* 网络请求监控 */}
				<div className="rounded-lg border bg-white p-6">
					<h2 className="mb-4 font-semibold text-xl">🌐 网络请求监控</h2>
					<button
						...
						<table className="min-w-full divide-y divide-gray-200">
							...
							<tbody className="divide-y divide-gray-200 bg-white">
								{networkRequests.map((request, index) => (
									<tr key={index}>
										<td className="whitespace-nowrap px-6 py-4 text-gray-900 text-sm">{request.url}</td>
										<td className="whitespace-nowrap px-6 py-4 text-gray-500 text-sm">{request.method}</td>
										<td className="whitespace-nowrap px-6 py-4 text-gray-500 text-sm">{request.duration}ms</td>
										<td className="whitespace-nowrap px-6 py-4 text-gray-500 text-sm">{request.status}</td>
									</tr>
								))}
							</tbody>
						</table>

						{networkRequests.length === 0 && <p className="py-4 text-center text-gray-500 text-sm">暂无网络请求记录</p>}
					</div>
				</div>

				{/* 自定义事务 */}
				<div className="rounded-lg border bg-white p-6">
					<h2 className="mb-4 font-semibold text-xl">🏷️ 自定义事务</h2>
					...
					<div className="space-y-2">
						{customTransactions.length === 0 ? (
							...
							customTransactions.map((transaction, index) => (
								<div key={index} className="rounded bg-blue-50 p-3 text-blue-800 text-sm">
									{transaction}
								</div>
							))
						)}
				...
				<div className="rounded-lg border bg-white p-6">
					<div className="mb-4 flex items-center justify-between">
						<h2 className="font-semibold text-xl">📝 测试结果</h2>
						<button onClick={clearResults} className="rounded bg-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-400">
							清空结果
						</button>
					</div>
				...
				{/* 性能优化建议 */}
				<div className="rounded-lg border bg-white p-6">
					<h2 className="mb-4 font-semibold text-xl">💡 性能优化建议</h2>
					<div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
						<div>
							<h3 className="mb-2 font-semibold text-gray-900">前端优化</h3>
						...
						<div>
							<h3 className="mb-2 font-semibold text-gray-900">后端优化</h3>
							<ul className="list-inside list-disc space-y-1 text-gray-600">
								<li>优化数据库查询和索引</li>
								<li>实现 API 响应缓存</li>
								<li>使用 CDN 加速静态资源</li>
