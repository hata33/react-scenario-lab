...
export class TableExporter {
  ...
  static exportToAdvancedCsv(data: any[], options?: ExportOptions): string {
    ...
    for (let i = 0; i < data.length; i += CHUNK_SIZE) {
      ...
      if (i === 0 && includeHeaders) {
        ...
        csv += headers.map(header => this.escapeCsvValue(header)).join(delimiter) + '\n';
      }
  ...
  private static applyCellStyles(worksheet: any, options: ExportOptions): void {
    // 设置列宽
    const colWidths = Object.keys(worksheet).filter(key => key[0] === '!').map(() => 15);
    if (colWidths.length > 0) {
      worksheet['!cols'] = colWidths.map(width => ({ wch: width }));
    }

    // 设置行高
    const rowHeights = Object.keys(worksheet).filter(key => key[0] === '!').map(() => 20);
