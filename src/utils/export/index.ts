 ...
 * 数据过滤器
 */
export function filterData(data: any[], filters: ExportFilter[]): any[] {
 ...
 * 数据验证
 */
export function validateData(data: any): { valid: boolean; message?: string } {
 ...
 * 估算文件大小
 */
export function estimateFileSize(data: any, format: string): number {
	let content: string;
 ...
 * 估算导出时间
 */
export function estimateExportTime(dataSize: number, format: string): number {
	// 基础时间（毫秒）
	const baseTime = 100;
 ...
 * 生成唯一ID
 */
export function generateId(prefix = "export"): string {
