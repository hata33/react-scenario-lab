...
describe('Export Utilities', () => {
  ...
  describe('TableExporter', () => {
    test('should export to advanced CSV with chunking', () => {
      const largeData = Array(15000).fill(0).map((_, i) => ({
        id: i,
        name: `User ${i}`,
        value: i * 10
      }));
    ...
    test('should format CSV values correctly', () => {
      const data = [
        { text: 'Hello, world!', number: 42, date: new Date('2023-01-01'), boolean: true }
      ];
  ...
  describe('ExportUtils', () => {
    test('should validate data correctly', () => {
      expect(ExportUtils.validateData({ name: 'John' })).toEqual({ valid: true });
      expect(ExportUtils.validateData(null)).toEqual({ valid: false, message: '数据不能为空' });
      expect(ExportUtils.validateData({})).toEqual({ valid: false, message: '数据对象不能为空' });
      expect(ExportUtils.validateData([])).toEqual({ valid: false, message: '数据数组不能为空' });
    });

    test('should sanitize filename', () => {
      expect(ExportUtils.sanitizeFilename('file/name.txt')).toBe('file_name.txt');
...
describe('Export Integration', () => {
  test('should handle complex data structures', () => {
    const complexData = {
      users: [
        { id: 1, name: 'John', profile: { age: 30, skills: ['JavaScript', 'React'] } },
        { id: 2, name: 'Jane', profile: { age: 25, skills: ['Python', 'Django'] } }
      ],
      metadata: {
        total: 2,
  ...
  test('should handle edge cases', () => {
    ...
    expect(csvResult).toContain('"Special: ""quotes"", \'apostrophes\', &symbols"');
  });
  ...
  test('should handle large datasets', () => {
    ...
    const jsonResult = TextExporter.exportToJson(largeData, { template: 'minified' });
