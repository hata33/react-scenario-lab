...
export class ImageExporter {
   ...
   * 导出Canvas为图片
   */
  static exportCanvasToImage(canvas: HTMLCanvasElement, format: 'png' | 'jpg' | 'webp', options?: ExportOptions): Promise<Blob> {
   ...
   * 转换图片格式
   */
  static async convertFormat(blob: Blob, fromFormat: string, toFormat: 'png' | 'jpg' | 'webp', options?: ExportOptions): Promise<Blob> {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.onload = () => {
        const canvas = document.createElement('canvas');
