...
export default function FeatureThreeWRule({ title, sections }: FeatureThreeWRuleProps) {
	return (
		<div className="rounded-lg border border-blue-200 bg-blue-50 p-4 md:p-6 shadow-sm">
			<h2 className="mb-4 break-words md:mb-6 font-bold text-xl md:text-2xl text-blue-800">{title}</h2>
					...
					return (
						<div key={index} className={`rounded-lg border ${config.borderColor} bg-white p-3 md:p-4 shadow-sm`}>
							<div className="mb-2 md:mb-3 flex items-center gap-2">
								<div
									className={`flex h-7 w-7 md:h-8 md:w-8 shrink-0 items-center justify-center rounded-full ${config.buttonBgColor} font-bold text-white text-sm`}
								>
									W
								</div>
								<h3 className={`text-sm font-semibold md:text-base ${config.titleColor}`}>{config.title}</h3>
							</div>
							<p className={`${config.descriptionColor} mb-2 md:mb-3 break-words text-sm leading-relaxed`}>{section.description}</p>
							<div className={`rounded border ${config.borderColor} bg-white p-2 md:p-3`}>
								<h4 className={`mb-1 md:mb-2 break-words text-xs font-medium md:text-sm ${config.featureTitleColor}`}>核心特性</h4>
