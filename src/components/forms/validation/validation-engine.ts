...
export class ValidationEngine {
  ...
  // 防抖验证
  private debounceValidation(fieldPath: string, context: ValidationContext, delay: number = 300): Promise<ValidationResult> {
    ...
    while (this.validationQueue.length > 0) {
      ...
      try {
        const results = await Promise.all(
          task.rules.map(rule =>
            this.executeAsyncRule(rule, task.context.formData[task.fieldPath], task.context)
          )
        );
