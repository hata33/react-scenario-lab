...
export class ConditionEvaluator {
  ...
  static evaluateConditions(
    ...
    return logic === 'and'
      ? conditions.every(condition => this.evaluate(condition, formData))
      : conditions.some(condition => this.evaluate(condition, formData));
  }
  ...
  static getFieldVisibility(
    ...
    if (dependency.hideWhen) {
      const shouldHide = this.evaluateConditions(dependency.hideWhen, formData, dependency.hideWhen[0]?.logic || 'and');
      if (shouldHide) return false;
    }

    // 检查显示条件
    if (dependency.showWhen) {
      const shouldShow = this.evaluateConditions(dependency.showWhen, formData, dependency.showWhen[0]?.logic || 'and');
      return shouldShow;
    }
  ...
  static getFieldDisabled(
    ...
    if (dependency.disableWhen) {
      const shouldDisable = this.evaluateConditions(dependency.disableWhen, formData, dependency.disableWhen[0]?.logic || 'and');
      if (shouldDisable) return true;
    }

    // 检查启用条件
    if (dependency.enableWhen) {
      const shouldEnable = this.evaluateConditions(dependency.enableWhen, formData, dependency.enableWhen[0]?.logic || 'and');
      return !shouldEnable;
    }
  ...
  static handleFieldUpdates(
    ...
    for (const update of dependency.updateWhen) {
      const shouldUpdate = this.evaluate({
        field: update.field,
